...
5. **Fail closed.** Unsupported schemas, duplicate identities and corrupted evidence packages stop compare/package workflows instead of producing partial results.
...
Fallback fingerprints use logical scope, category, family/type, system/material, level, mark, size and approximate location. Fingerprint-based matches are never treated as exact identity.

## Offline boundary

`tools/revit_estimating.py` exposes validate/compare/package workflows without Revit. Golden snapshot fixtures exercise the same diff engine used by the pyRevit command. This allows most transformation, identity, integrity and export behavior to be tested in CI; only the Autodesk API adapter and actual ribbon execution require a live Revit environment.
